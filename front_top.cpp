...
void front_top(struct front_top_in *in, struct front_top_out *out) {
  ...
  bpu_in.refetch_valid = in->refetch_valid;
  ...
  // set FIFO input
  fifo_in.icache_read_ready = icache_out.icache_read_ready;
  for (int i = 0; i < FETCH_WIDTH; i++) {
    fifo_in.fetch_group[i] = icache_out.fetch_group[i];
  }
  fifo_in.mispredict = in->refetch_valid;

  // run FIFO
  instruction_FIFO_top(&fifo_in, &fifo_out);

  // set PTAB input
  ptab_in.PTAB_read_enable = fifo_out.PTAB_read_enable;
  ptab_in.PTAB_write_enable = fifo_out.PTAB_write_enable;
  ...
  ptab_in.mispredict = in->refetch_valid;
