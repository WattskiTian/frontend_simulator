...
#define TN_IDX_MASK (TN_ENTRY_NUM - 1)
...
uint32_t cal_index(uint32_t PC, int n) {
  uint32_t ret = FH[0][n] ^ (PC >> 2) & (TN_IDX_MASK);
  return ret;
}
...
void TAGE_do_update(uint32_t PC, bool real_dir, pred_out pred_out) {
  ...
  uint32_t row_cnt = (u_clear_cnt >> 11) & (TN_IDX_MASK);
...
void print_IO_data(uint32_t base_pc) {
  ...
  for (int i = 0; i < TN_MAX; i++) {
    tmp_tage_idx[i] = FH[0][i] ^ (base_pc >> 2) & (TN_IDX_MASK);
  }
  for (int i = 0; i < TN_MAX; i++) {
...
int main() {
  ...
  while (log_pc_max--) {
    ...
    inst_cnt++;
    pred_out = TAGE_get_prediction(log_pc);
    print_IO_data(log_pc);
    printf("%d\n", log_dir);
    TAGE_do_update(log_pc, log_dir, pred_out);
    if (show_details == true) {
      printf("TAGE_bp = %d", pred_out.pred);
      if (pred_out.pred == log_dir)
