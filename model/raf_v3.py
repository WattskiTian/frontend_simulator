...
# 训练随机森林
def train_random_forest(num_samples=None):
    # 加载数据
    ...
    # 保存特征重要性
    feature_importances_filename = 'feature_importances_2.txt'
    save_feature_importances(rf_classifier, feature_importances_filename)

    # 保存树决策规则
    tree_rules_filename = 'tree_rules_2.txt'
...
if __name__ == "__main__":
    # model = train_random_forest(num_samples=1000)  # 例如，只加载 1000 个样本
    model = train_random_forest()  # 例如，只加载 1000 个样本